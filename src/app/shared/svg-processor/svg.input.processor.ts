...
export namespace IProcessor {
    ...
    export namespace PathProcessor {
        class Command {
            constructor(
        ...
        export function convertElement(oldSvgElement: Element): Element {
        ...
        function buildNewSvgElement(oldSvgElement: Element, dIn: String, convertedPoints: [number, number][]): Element {
            const newSvgElement = <Element>oldSvgElement.cloneNode();
    ...
    function extractValues(rawValues: string): number[] {
        const values: number[] = []
